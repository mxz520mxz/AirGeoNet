...
class ALLDataModule(pl.LightningDataModule):
    default_cfg = {
        **MapLocDataset.default_cfg,
        "name": "ALL",
        # paths and fetch
        "data_dir": DATASETS_PATH / "ALL",
        "tiles_filename": "tiles.pkl",
        "qiyuan_map":"qiyuan.pkl",
        "subdataset":"???",
        "splits": {
        ...
        "have_qiyuan_map":False,
    }
    dummy_scene_name = "ALL"
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        for subset in self.cfg.subdataset:
            
            splits_subset = {}
            others_subset = {}
            for stage in stages:
                key = subset + '_' + self.cfg.splits[stage]
                splits_subset[stage], others_subset[stage] = self.parse_split(key)
            self.all_splits[subset] = copy.deepcopy(splits_subset)
            self.all_others[subset] = copy.deepcopy(others_subset)
        ...
        if self.tile_manager is None:
            tile_managers = {}
            for subset in self.cfg.subdataset:
                logger.info("Loading the tile " + subset +" manager... ")
                tile_manager = TileManager.load(self.root / Path(subset + '_'+ self.cfg.tiles_filename))
                tile_managers[subset] = tile_manager
            self.tile_manager = tile_managers
            


        #TODO
        self.cfg.num_classes = {k: len(g) for k, g in self.tile_manager[self.cfg.subdataset[0]].groups.items()}
        self.cfg.pixel_per_meter = self.tile_manager[self.cfg.subdataset[0]].ppm

        #load qiyuan map
        if self.cfg.have_qiyuan_map:
            logger.info("Loading the qiyuan map...")
            with open(self.root / self.cfg.qiyuan_map, 'rb') as f:
                self.qiyuan_map = pickle.load(f)
    ...
    def pack_data(self, stages):
        ...
        for stage in stages:
            for subset in self.cfg.subdataset:
                ...
                for path in enumerate(self.all_splits[subset][stage]):
                    names.append(path[1])
                for other in enumerate(self.all_others[subset][stage]):
                    other = other[1]
                    gps_position.append([other[0],other[1]])
