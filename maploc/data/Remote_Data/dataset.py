...
class RemoteDataModule(pl.LightningDataModule):
    default_cfg = {
        **MapLocDataset.default_cfg,
        "name": "Remote",
        # paths and fetch
        "data_dir": DATASETS_PATH / "Remote",
    ...
    def prepare_data(self):
        if not (self.root.exists()):
            raise FileNotFoundError(
                "Cannot find the Remote Data dataset"
            )
    ...
    def pack_data(self, stages):
        for stage in stages:
            ...
            for other in enumerate(self.others[stage]):
                ...
                euler.append([other[2],other[3],other[4]])
