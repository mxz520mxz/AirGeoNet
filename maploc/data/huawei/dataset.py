...
class HuaweiDataModule(pl.LightningDataModule):
    default_cfg = {
        **MapLocDataset.default_cfg,
        "name": "huawei",
        # paths and fetch
        "data_dir": DATASETS_PATH / "huawei",
    ...
    def prepare_data(self):
        if not (self.root.exists()):
            raise FileNotFoundError(
                "Cannot find the Huawei dataset"
            )
    ...
    def pack_data(self, stages):
        for stage in stages:
            ...
            for path in enumerate(self.splits[stage]):
                name = '/'.join(path[1])
                names.append(name)
                [lat,lon,alt] = self.extract_gps_info(name)
                
                gps_position.append([float(lat),float(lon),float(alt)])
