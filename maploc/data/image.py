...
def rectify_image(
    ...
    *_, h, w = image.shape
    ...
    grid = torch.stack((grid_x, grid_y), dim=-1).to(image.dtype)

    # grid = make_grid(w,h,device=image.device).to(image.dtype)
    ...
    # print(grid_norm[None].shape)
    # print(depth_image.unsqueeze(0)[None].shape)
