...
def random_rot90(raster, xy, heading, seed=None, region=None):
    ...
    if region is not None:
        region = np.rot90(region, rot, axes=(-0, -1))
       
    return raster, region, xy2, heading


# def random_flip(image, depth_image, raster, xy, heading, seed=None):
#     state = np.random.RandomState(seed)
...
#     return image, depth_image, raster, xy, heading

def random_flip(image, raster, xy, heading, seed=None, region=None):
    state = np.random.RandomState(seed)
    if state.rand() > 0.5:  # no flip
        return image, raster, region, xy, heading
    image = image[:, ::-1]
