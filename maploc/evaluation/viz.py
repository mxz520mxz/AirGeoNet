...
def plot_example_single(
    ...
    scene = 'huawei'
    name, rasters, raster_qiyuan, uv_gt = (data[k] for k in ( "name", "map", "qiyuan_map", "uv"))
    ...
    if raster_qiyuan[0] is not None:
        map_viz = raster_qiyuan.cpu().numpy()
    else:
        map_viz = Colormap.apply(rasters)
        
    # prob = F.interpolate(prob.unsqueeze(0).unsqueeze(0), scale_factor=2, mode='bilinear', align_corners=False).squeeze(0).squeeze(0)
    # print('prob.numpy() shape:',prob.numpy().shape)
    ...
    if out_dir is not None:
        ...
        p = out_dir + '/' + f"{scene}_{name_}_{{}}.pdf"
    ...
    feats_q = pred["features_air"]
    
    prior = None
    if "log_prior" in pred["map"]:
        prior = pred["map"]["log_prior"][0].sigmoid()
    if "air" in pred and "confidence" in pred["air"]:
        conf_q = pred["air"]["confidence"]
    else:
        conf_q = torch.norm(feats_q, dim=0)
   
    (feats_q_rgb,) = features_to_RGB(feats_q.numpy())
    
    # norm_map = torch.norm(feats_map, dim=0)

    plot_images(
        [conf_q, feats_q_rgb] + ([] if prior is None else [prior]),
        titles=["Air confidence", "Air features"]
