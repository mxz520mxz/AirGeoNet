...
class TemplateSampler(torch.nn.Module):
    def __init__(self, device, num_rotations, optimize=False):
        super().__init__()

        if optimize:
            assert (num_rotations % 4) == 0
            angles = torch.arange(
                0, 90, 90 / (num_rotations // 4), device = device
            )
        else:
            angles = torch.arange(
                0, 360, 360 / num_rotations, device = device
            )
    ...
    def forward(self, image_bev):
        ...
        for angle in self.angles:
            rotated_image = rotate(image_bev, -angle.item())
        ...
        image = torch.stack(image).to(image_bev.device)
        if image.dim() == 5:
