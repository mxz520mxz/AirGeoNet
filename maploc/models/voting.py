...
class TemplateSampler(torch.nn.Module):
    ...
    def forward(self, image_air):
        image = []
        for angle in self.angles:
            rotated_image = rotate(image_air, -angle.item())
            rotated_image = F.interpolate(rotated_image, size=(rotated_image.shape[2]+1, rotated_image.shape[3]+1), mode='bilinear', align_corners=False)
            image.append(rotated_image.squeeze(0))
        image = torch.stack(image)
        if image.dim() == 5:
            kernels = image.permute(1,0,2,3,4)
        else:
            kernels = image.unsqueeze(0)
    
        if self.optimize:  # we have computed only the first quadrant
            kernels_quad234 = [torch.rot90(kernels, -i, (-2, -1)) for i in (1, 2, 3)]
            print('kernels_quad234 shape:',kernels_quad234[0].shape)
            plt.imsave('rotate1.jpg',np.sum(kernels_quad234[0][0][0].cpu().numpy(),axis=0))
            kernels_quad234 = [kernels_quad234[0].permute(0,1,2,4,3)]
            plt.imsave('rotate2.jpg',np.sum(kernels_quad234[0][0][0].cpu().numpy(),axis=0))
...
def conv2d_fft_batchwise(signal, kernel, padding="same", padding_mode="constant"):
    ...
    signal_fr = rfftn(signal.cpu().numpy(), axes=(-1, -2))
    kernel_fr = rfftn(kernel_padded.cpu().numpy(), axes=(-1, -2))

    kernel_fr.imag *= -1  # flip the kernel
    output_fr = torch.einsum("bc...,bdc...->bd...", torch.tensor(signal_fr), torch.tensor(kernel_fr))
    output = torch.tensor(irfftn(output_fr.cpu().numpy(), axes=(-1, -2)))
...
def expectation_xyr(
    ...
    # grid_xy = make_grid(float(w), float(h), device=prob.device)

    x = torch.arange(w, device=prob.device, dtype=prob.dtype)
    y = torch.arange(h, device=prob.device, dtype=prob.dtype)

    # Manually create the grid using broadcasting
    grid_x = x.unsqueeze(0).expand(h, w)
    grid_y = y.unsqueeze(1).expand(h, w)

    # Stack the grids along the last dimension
    grid_xy = torch.stack((grid_x, grid_y), dim=-1).to(prob.dtype)
