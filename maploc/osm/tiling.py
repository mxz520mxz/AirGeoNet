...
class TileManager:
    ...
    def load(cls, path: Path):
        ...
        def process_tile(ij, bbox, ppm, raster_path):
            canvas = Canvas(BoundaryBox.from_string(bbox), ppm)
            try:
                raster = np.asarray(Image.open(raster_path))
                canvas.raster = raster.transpose(2, 0, 1).copy()
            except Exception as e:
                # Handle the exception (e.g., log the error) and continue or raise an error.
                print(f"Error loading raster for {ij}: {e}")
            return ij, canvas
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            for ij, bbox in dump["tiles_bbox"].items():
                ppm = dump["ppm"]
                raster_path = dump["tiles_raster"][ij]
                futures.append(executor.submit(process_tile, ij, bbox, ppm, raster_path))

            for future in concurrent.futures.as_completed(futures):
                try:
                    ij, canvas = future.result()
                    tiles[ij] = canvas
                except Exception as e:
