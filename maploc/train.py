...
def train(cfg: DictConfig, job_id: Optional[int] = None):
    ...
    if init_checkpoint_path is not None:
        ...
        experiment = init_checkpoint_path
        path = resolve_checkpoint_path(experiment)

        cfg_ = {'model': {"num_rotations": 32, "apply_map_prior": True, "z_max": 128,"x_max": 128}}
        model = GenericModule.load_from_checkpoint(
            path, strict=False, find_best=not experiment.endswith('.ckpt'), cfg=cfg_)
    else:
...
@hydra.main(
    config_path=osp.join("conf"), config_name="orienternet"
)
...
if __name__ == "__main__":
    print('__file__',osp.join(osp.dirname(__file__), "conf"))
    main()
