...
class Projection:
    def __init__(self, lat, lon, alt=0, max_extent=35e3):
    ...
    @classmethod
    def from_latlonalt(cls, all_latlonalt):
        assert all_latlonalt.shape[-1] == 3
        latlonalt_mid = (np.max(all_latlonalt,axis=0) + np.min(all_latlonalt,axis=0)) / 2
        return cls(*latlonalt_mid)
